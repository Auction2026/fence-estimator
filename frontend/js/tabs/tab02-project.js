...
/**
 * Project details tab helpers for job setup, type selection, date coordination, and job number handling.
...
function parseStoredValue() {
    try {
        ...
        console.warn('project storage parse failed.', error);
        return {};
    }
}
...
function validate(state, options) {
    ...
    if (!String(state.siteAddress || '').trim()) {
        return { valid: false, message: 'Site address is required.' };
    }
...
const api = {
    init: initTab02Project,
    initTab02Project: initTab02Project
};
