...
(() => {
  const TAB_ID = 'estimate';
  const FORM_ID = "estimate-controls-form";
  ...
  function updateSummary(ctx) {
    const values = readValues();
    if (TAB_ID === 'project') {
      ctx.ui.renderSummaryList('project-summary', [['taxRate', values['taxRate'] || ''], ['contingencyRate', values['contingencyRate'] || ''], ['laborRate', values['laborRate'] || '']]);
    }
  }
