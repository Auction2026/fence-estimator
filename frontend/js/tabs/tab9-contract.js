...
(() => {
  ...
  function updateSummary(ctx) {
    const values = readValues();
    if (TAB_ID === 'project') {
      ctx.ui.renderSummaryList('project-summary', [['priceLockDays', values['priceLockDays'] || ''], ['depositPercent', values['depositPercent'] || ''], ['paymentTerms', values['paymentTerms'] || '']]);
    }
  }
