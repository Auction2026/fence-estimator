...
'use strict';
/**
 * Measurement tab helpers for linear footage, sections, gates, corners, terrain, soil, and an interactive calculator.
 * @module tab04Measurements
 */
...
function parseStoredValue() {
    ...
    catch (error) { console.warn('measurements storage parse failed.', error); return {}; }
}
...
function validate(state, options) {
    ...
    if (asNumber(state.linearFeet, 0) <= 0) { return { valid: false, message: 'Linear feet must be greater than zero.' }; }
...
function render(root, elements, state) {
    ...
    if (calculator) { calculator.textContent = baseFeet + ' LF'; }
    if (adjusted) { adjusted.textContent = adjustedFeet.toFixed(1) + ' adjusted LF'; }
...
function applyTerrainFactor(feet, terrainType) { return feet * (TERRAIN_FACTORS[terrainType] || 1) * (SOIL_FACTORS[parseStoredValue().soilType] || 1); }

const api = { init: initTab04Measurements, initTab04Measurements: initTab04Measurements };
