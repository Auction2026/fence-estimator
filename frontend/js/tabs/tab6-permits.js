...
(() => {
  const TAB_ID = 'permits';
  const FORM_ID = "permits-form";
  ...
  function updateSummary(ctx) {
    const values = readValues();
    if (TAB_ID === 'project') {
      ctx.ui.renderSummaryList('project-summary', [['permitRequired', values['permitRequired'] || ''], ['permitType', values['permitType'] || ''], ['permitAuthority', values['permitAuthority'] || '']]);
    }
  }
