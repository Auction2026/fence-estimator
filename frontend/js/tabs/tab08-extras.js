...
/**
 * Extras and options helpers for add-ons, demolition, permits, custom line items, discount codes, and totals.
...
function parseStoredValue() {
    try {
        ...
        console.warn('extras storage parse failed.', error);
        return {};
    }
}
...
function render(root, elements, state) {
    ...
    if (total) {
        total.textContent = asCurrency(state.extrasTotal);
    }
...
const DISCOUNT_CODES = { SAVE5: 0.05, SAVE10: 0.1, COMMERCIAL15: 0.15 };

function calculateExtrasTotal(state) {
    const base = Object.keys(EXTRA_COSTS).reduce(function (total, key) {
        return total + (state[key] ? EXTRA_COSTS[key] : 0);
    }, 0) + (String(state.customItemName || '').trim() ? asNumber(state.customItemPrice, 0) : 0);
    const discount = DISCOUNT_CODES[String(state.discountCode || '').trim().toUpperCase()] || 0;
    return Math.max(0, base - (base * discount));
}

const api = {
    init: initTab08Extras,
    initTab08Extras: initTab08Extras
};
