...
/**
 * Fence type configuration helpers for product family, height, finish, style, and grade choices.
...
function parseStoredValue() {
    try {
        ...
        console.warn('fence type storage parse failed.', error);
        return {};
    }
}
...
function handleInput(root, elements, event) {
    ...
    if (target && (target.id === 'fenceType' || target.name === 'fenceType')) {
        applyFencePreset(root, state.fenceType);
    }
...
function validate(state, options) {
    ...
    if (!String(state.fenceType || '').trim()) {
        return { valid: false, message: 'Select a fence type.' };
    }

    if (asNumber(state.height, 0) <= 0) {
        return { valid: false, message: 'Select a valid fence height.' };
    }
...
function render(root, elements, state) {
    ...
    if (summary) {
        summary.textContent = [state.fenceType, state.height + ' ft', state.color, state.style, state.grade].join(' • ');
    }
...
const api = {
    init: initTab03FenceType,
    initTab03FenceType: initTab03FenceType
};
