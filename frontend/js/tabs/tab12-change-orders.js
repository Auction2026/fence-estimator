...
(() => {
  const TAB_ID = 'change-orders';
  const FORM_ID = "change-order-form";
  ...
  function bindPrimaryButton(ctx) {
    const buttonId = "addChangeOrderBtn";
    if (!buttonId) {
      return;
    }
  ...
  function updateSummary(ctx) {
    const values = readValues();
    if (TAB_ID === 'project') {
      ctx.ui.renderSummaryList('project-summary', [['changeOrderDate', values['changeOrderDate'] || ''], ['changeOrderCost', values['changeOrderCost'] || ''], ['changeOrderReason', values['changeOrderReason'] || '']]);
    }
  }
