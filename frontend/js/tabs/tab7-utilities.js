...
(() => {
  const TAB_ID = 'utilities';
  const FORM_ID = "utilities-form";
  ...
  function updateSummary(ctx) {
    const values = readValues();
    if (TAB_ID === 'project') {
      ctx.ui.renderSummaryList('project-summary', [['locateTicket', values['locateTicket'] || ''], ['locateStatus', values['locateStatus'] || ''], ['locateExpiry', values['locateExpiry'] || '']]);
    }
  }
