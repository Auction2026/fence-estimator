...
'use strict';
/**
 * Summary tab helpers that consolidate materials, labor, extras, tax, grand total, and print or export actions.
 * @module tab09Summary
 */
...
function parseStoredValue() {
    ...
    catch (error) { console.warn('summary storage parse failed.', error); return {}; }
}
...
function validate(state, options) {
    ...
    if (asNumber(state.taxRate, -1) < 0) { return { valid: false, message: 'Tax rate cannot be negative.' }; }
...
function render(root, elements, state) {
    ...
    if (taxNode) { taxNode.textContent = asCurrency(state.taxAmount); }
    if (totalNode) { totalNode.textContent = asCurrency(state.grandTotal); }
    ...
    if (exportButton && !exportButton.dataset.bound) {
        ...
        exportButton.addEventListener('click', function () { emit('fence-estimator:summary-export', collectState(root)); });
    }
...
const api = { init: initTab09Summary, initTab09Summary: initTab09Summary };
